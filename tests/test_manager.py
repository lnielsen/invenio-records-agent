...
from invenio_records_agent import RecordManager, RecordManagerFactory


def test_manager_get(manager_cls, identity_simple, fake_record_db):
    """Get a record."""
    for recid, fake_record in fake_record_db.items():
        recstate = manager_cls.get(recid, identity=identity_simple)
